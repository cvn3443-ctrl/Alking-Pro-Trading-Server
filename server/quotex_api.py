...
class QuotexAPI:
    """عميل API مباشر لـ Quotex - بدون متصفح"""
    ...
    def __init__(self):
        ...
        # إعدادات الـ Headers لمحاكاة متصفح حقيقي
    ...
    def login(self, email: str, password: str) -> Dict:
        """تسجيل الدخول إلى Quotex عبر API"""
        try:
            logger.info(f"محاولة تسجيل الدخول: {email}")
            
            # نقطة نهاية تسجيل الدخول الفعلية لـ Quotex
            login_url = "https://qxbroker.com/api/v1/login"
            ...
            if response.status_code == 200:
                ...
                if data.get('success') or data.get('token'):
                    ...
                    # إضافة التوكن للطلبات المستقبلية
                    self.session.headers.update({
    ...
    def _fetch_symbols(self):
        """جلب العملات المتاحة"""
        try:
            symbols_url = "https://qxbroker.com/api/v1/assets"
            response = self.session.get(symbols_url, timeout=15)
